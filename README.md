...
3. 한 화면에서 선택한 모든 센서의 **측정 항목과 필드 이름**을 확인합니다.
   측정 항목은 센서의 종류와 단위로 미리 채워집니다.
...
- % 단위는 배터리·필터 잔량 등에도 쓰이므로 `device_class`가 습도
  (`humidity`)나 수분(`moisture`)인 센서만 목록에 나타납니다. 많은 토양센서가
  토양습도를 습도로 보고하므로 두 경우 모두 `02`와 `07` 중에서 고를 수 있습니다.
...
| 원하는 센서가 목록에 없음 | 개발자 도구 → 상태에서 센서의 `unit_of_measurement`와 `device_class`를 확인합니다. 위 표의 단위가 아니거나 % 센서에 `device_class`가 없으면 템플릿 센서로 단위와 `device_class`를 맞춥니다. |
